...
#include "AudioRTDSP_i24_2ch.hpp"
#include <combaseapi.h>

AudioRTDSP_i24_2ch::AudioRTDSP_i24_2ch(const audiortdsp_pb_params_t *p_params) : AudioRTDSP(p_params)
{
}

AudioRTDSP_i24_2ch::~AudioRTDSP_i24_2ch(VOID)
{
...
BOOL WINAPI AudioRTDSP_i24_2ch::audio_hw_init(VOID)
{
	HRESULT n_ret;
	UINT32 u32;
	...
	{
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: p_audiodev is NULL.");
		return FALSE;
	}
	...
	if(n_ret != S_OK)
	{
		...
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: IMMDevice::Activate failed.");
		return FALSE;
	}
	...
	*/
	...
	{
		this->audio_hw_deinit_device();
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: audio format is not supported.");
		return FALSE;
	}
	...
	{
		this->audio_hw_deinit_device();
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: IAudioClient::Initialize failed.");
		return FALSE;
	}
	...
	{
		this->audio_hw_deinit_device();
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: IAudioClient::GetBufferSize failed.");
		return FALSE;
	}
	...
	{
		this->audio_hw_deinit_device();
		this->err_msg = TEXT("AudioRTDSP_i24_2ch::audio_hw_init: Error: IAudioClient::GetService failed.");
		return FALSE;
	}
...
BOOL WINAPI AudioRTDSP_i24_2ch::buffer_alloc(VOID)
{
	SIZE_T n_seg = 0u;
...
VOID WINAPI AudioRTDSP_i24_2ch::buffer_free(VOID)
{
	if(this->p_bufferinput != NULL)
	{
...
VOID WINAPI AudioRTDSP_i24_2ch::buffer_load(VOID)
{
	SIZE_T n_sample = 0u;
	SIZE_T n_byte = 0u;
...
VOID WINAPI AudioRTDSP_i24_2ch::buffer_play(VOID)
{
	SIZE_T n_sample = 0u;
	HRESULT n_ret = 0;

	n_ret = this->p_audioout->GetBuffer((UINT32) this->AUDIOBUFFER_SEGMENT_SIZE_FRAMES, (BYTE**) &(this->p_audiobuffer));
	if(n_ret != S_OK) app_exit(1u, TEXT("AudioRTDSP_i24_2ch::buffer_play: Error: IAudioRenderClient::GetBuffer failed."));

	for(n_sample = 0u; n_sample < this->AUDIOBUFFER_SEGMENT_SIZE_SAMPLES; n_sample++)
		((INT16*) this->p_audiobuffer)[n_sample] = (INT16) (((INT32*) (this->pp_bufferout_segments[this->bufferout_nseg_play]))[n_sample] & 0xffff);

	n_ret = this->p_audioout->ReleaseBuffer((UINT32) this->AUDIOBUFFER_SEGMENT_SIZE_FRAMES, 0u);
	if(n_ret != S_OK) app_exit(1u, TEXT("AudioRTDSP_i24_2ch::buffer_play: Error: IAudioRenderClient::ReleaseBuffer failed."));

	return;
}

VOID WINAPI AudioRTDSP_i24_2ch::dsp_proc(VOID)
{
	...
	{
		p_loadout_seg[2u*currin_seg_nframe] = p_currin_seg[2u*currin_seg_nframe];
		p_loadout_seg[2u*currin_seg_nframe + 1u] = p_currin_seg[2u*currin_seg_nframe + 1u];
		...
		while(n_cycle <= fx_params.n_feedback)
		{
			...
			p_loadout_seg[2u*currin_seg_nframe] += pol*(p_bufferin[2u*previn_buf_nframe])/cycle_div;
			p_loadout_seg[2u*currin_seg_nframe + 1u] += pol*(p_bufferin[2u*previn_buf_nframe + 1u])/cycle_div;

			n_cycle++;
		}

		p_loadout_seg[2u*currin_seg_nframe] /= 512;
		p_loadout_seg[2u*currin_seg_nframe + 1u] /= 512;

		if(p_loadout_seg[2u*currin_seg_nframe] > this->SAMPLE_MAX_VALUE_I16) p_loadout_seg[2u*currin_seg_nframe] = this->SAMPLE_MAX_VALUE_I16;
		else if(p_loadout_seg[2u*currin_seg_nframe] < this->SAMPLE_MIN_VALUE_I16) p_loadout_seg[2u*currin_seg_nframe] = this->SAMPLE_MIN_VALUE_I16;

		if(p_loadout_seg[2u*currin_seg_nframe + 1u] > this->SAMPLE_MAX_VALUE_I16) p_loadout_seg[2u*currin_seg_nframe + 1u] = this->SAMPLE_MAX_VALUE_I16;
		else if(p_loadout_seg[2u*currin_seg_nframe + 1u] < this->SAMPLE_MIN_VALUE_I16) p_loadout_seg[2u*currin_seg_nframe + 1u] = this->SAMPLE_MIN_VALUE_I16;
	}
