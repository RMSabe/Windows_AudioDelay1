...
BOOL WINAPI window_enable_vscroll(HWND p_wnd, BOOL enable)
{
	...
	return SetWindowLongPtr(p_wnd, GWL_STYLE, (LONG_PTR) wnd_style);
}
...
BOOL WINAPI window_set_parent(HWND p_wnd, HWND p_parent)
{
	...
	return SetWindowLongPtr(p_wnd, GWLP_HWNDPARENT, (LONG_PTR) p_parent);
}
...
BOOL WINAPI choosefile_proc(VOID)
{
	...
	if(textlen >= 5u)
		if(cstr_compare(TEXT(".wav"), &textbuf[textlen - 4u]))
			goto _l_choosefile_proc_filecheckext_complete;
...
_l_choosefile_proc_filecheckext_complete:
	...
	{
		case PB_I16_1CH:
			p_audio = new AudioRTDSP_i16_1ch(&pb_params);
			break;

		case PB_I16_2CH:
			p_audio = new AudioRTDSP_i16_2ch(&pb_params);
			break;

		case PB_I24_1CH:
			p_audio = new AudioRTDSP_i24_1ch(&pb_params);
			break;

		case PB_I24_2CH:
			p_audio = new AudioRTDSP_i24_2ch(&pb_params);
			break;
	}
...
INT WINAPI filein_get_params(VOID)
{
	...
	if(!compare_signature("WAVE", (const CHAR*) &p_headerinfo[8]))
	{
		tstr = TEXT("Error: file format not supported.");
		goto _l_filein_get_params_error;
	}
	...
	while(TRUE)
	{
		...
		p_u32 = (UINT32*) &p_headerinfo[bytepos + 4u];
		bytepos += (SIZE_T) (*p_u32 + 8u);
	}

	p_u16 = (UINT16*) &p_headerinfo[bytepos + 8u];
	...
	p_u32 = (UINT32*) &p_headerinfo[bytepos + 12u];
	pb_params.sample_rate = *p_u32;

	p_u16 = (UINT16*) &p_headerinfo[bytepos + 22u];
	bit_depth = *p_u16;
	...
	pb_params.audio_data_begin = (ULONG64) (bytepos + 8u);
	pb_params.audio_data_end = pb_params.audio_data_begin + ((ULONG64) *p_u32);
