...
BOOL WINAPI create_mainwnd(VOID)
{
	...
	return (BOOL) SetWindowLongPtr(p_mainwnd, GWLP_WNDPROC, (LONG_PTR) &mainwnd_wndproc);
}
...
BOOL WINAPI window_enable_vscroll(HWND p_wnd, BOOL enable)
{
	...
	return (BOOL) SetWindowLongPtr(p_wnd, GWL_STYLE, (LONG_PTR) wnd_style);
}
...
BOOL WINAPI window_set_parent(HWND p_wnd, HWND p_parent)
{
	...
	return (BOOL) SetWindowLongPtr(p_wnd, GWLP_HWNDPARENT, (LONG_PTR) p_parent);
}
...
INT WINAPI filein_get_params(VOID)
{
	...
	n_channels = *((UINT16*) (((SIZE_T) p_headerinfo) + buffer_index + 10u));
